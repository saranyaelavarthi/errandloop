...
function setupSteps(){
  ...
  return `<section class="setup-steps" aria-label="Your first circle"><div class="eyebrow">Your first circle</div><h2>Give a hand. Get a hand.</h2><p>Each participant needs both a trip and a request. Start with these three steps.</p><div class="setup-grid">${steps.map((step,i)=>`<div class="setup-step ${step.done?'done':''}"><span aria-hidden="true">${step.done?'✓':i+1}</span><strong>${step.label}${step.done?' — done':''}</strong><p>${step.detail}</p>${!step.done&&step.action==='group'?`<button class="button small" data-action="group">Invite a neighbour</button>`:''}</div>`).join('')}</div></section>`;
}
...
function pocket(){
  ...
  return `<div class="side-card"><h3>Your pocket <span aria-hidden="true">↘</span></h3>${ownTrip?`<div class="pocket-row">${placeIcon(ownTrip.destination)}<div><div class="overline">You’re heading to</div><strong>${esc(place(ownTrip.destination).name)}</strong><small>${time(ownTrip.depart)} – ${time(ownTrip.returns)}</small><small>${ownTrip.capacity} small item spaces</small></div></div><button class="subtle-link" data-action="post" data-id="${ownTrip.id}">View your trip</button>`:`<p class="card-note">No trip yet. Use “I’m heading out” to add your planned stop.</p>`}<div class="pocket-divider"></div>${ownNeed?`<div class="pocket-row">${icon('bag',21)}<div><div class="overline">You need a hand with</div><strong>${esc(ownNeed.item)}</strong><small>By ${time(ownNeed.deadline)}</small></div></div><button class="subtle-link" data-action="post" data-id="${ownNeed.id}">View your request</button>`:`<p class="card-note">No request yet. Use “I need a hand” to ask for a pickup.</p>`}</div>`;
}
