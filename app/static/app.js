...
function showLoop(id,focus=true){
  ...
  const currentStage=['expired','cancelled'].includes(l.status)?-1:stages.findIndex(stage=>!stage.done);
  ...
  openModal('Your circle, one step at a time.' ,statusNames[l.status],
    ...
    <div class="acceptance">${l.members.map(m=>`<span class="${l.accepted.includes(m)?'yes':''}">${l.accepted.includes(m)?'✓':'○'} ${esc(name(m))}</span>`).join('')}</div>${nextStep?`<div class="callout next-step" role="status">${esc(nextStep)}</div>`:''}
    ${l.status==='awaiting'?`<div class="actions">${!l.accepted.includes(actor)?`<button class="button primary" data-mutation data-action="accept" data-id="${id}">Yes, I’m in ${icon('check')}</button><button class="button" data-mutation data-action="decline" data-id="${id}">Decline</button>`:`<p class="guide-sub">You’ve accepted. Still waiting for ${esc(l.members.filter(m=>!l.accepted.includes(m)).map(m=>name(m)).join(' and '))}. Each person must accept from their own account.</p>`}</div><p class="guide-sub">Everyone must accept before ${time(l.expires)}.</p>`:''}
    ${l.status==='expired'?'<div class="callout warning">This circle expired before everyone accepted. It cannot move to collection. Create new posts with future times to arrange another exchange.</div>':''}
    ${edgesHTML(l,true)}
